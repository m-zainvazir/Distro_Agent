...
def _scalar_user(db: AsyncMock, hashed: str) -> None:
    ...
    user = User(id=_USER_ID, email=_EMAIL, hashed_password=hashed, tenant_id=_TENANT_ID)
    result = MagicMock()
...
async def test_login_correct_password_returns_jwt() -> None:
    ...
    _test_app.dependency_overrides[get_db] = _override
    async with AsyncClient(transport=ASGITransport(app=_test_app), base_url="http://test") as client:
        resp = await client.post("/api/v1/auth/login", json={"email": _EMAIL, "password": _PASSWORD})
    _test_app.dependency_overrides.clear()
...
async def test_login_wrong_password_returns_401() -> None:
    ...
    _test_app.dependency_overrides[get_db] = _override
    async with AsyncClient(transport=ASGITransport(app=_test_app), base_url="http://test") as client:
        resp = await client.post("/api/v1/auth/login", json={"email": _EMAIL, "password": "wrongpassword"})
    _test_app.dependency_overrides.clear()
...
async def test_login_unknown_email_returns_401() -> None:
    ...
    _test_app.dependency_overrides[get_db] = _override
    async with AsyncClient(transport=ASGITransport(app=_test_app), base_url="http://test") as client:
        resp = await client.post("/api/v1/auth/login", json={"email": "nobody@example.com", "password": _PASSWORD})
    _test_app.dependency_overrides.clear()
    assert resp.status_code == 401
...
async def test_get_current_tenant_valid_token() -> None:
    ...
    async def _protected(tenant: Tenant = pytest.importorskip("fastapi").Depends(get_current_tenant)) -> dict:
...
async def test_get_current_tenant_invalid_token_returns_401() -> None:
    ...
    dep_app.dependency_overrides[get_db] = _override_db
    async with AsyncClient(transport=ASGITransport(app=dep_app), base_url="http://test") as client:
        resp = await client.get("/protected", headers={"Authorization": "Bearer not-a-valid-token"})
    dep_app.dependency_overrides.clear()
    assert resp.status_code == 401
...
async def test_tenant_isolation_token_resolves_to_own_tenant_only() -> None:
    ...
    tenant_a_id, tenant_b_id = uuid.uuid4(), uuid.uuid4()
    ...
    @dep_app.get("/me")
    async def _me(tenant: Tenant = pytest.importorskip("fastapi").Depends(get_current_tenant)) -> dict:
