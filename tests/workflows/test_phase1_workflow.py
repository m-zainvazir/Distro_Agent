"""
Validation tests for the Phase 1 orchestration workflow.

Patch targets:
  - extract_brand  → app.workflows.phase1_workflow.extract_brand
  - scout_stores   → app.workflows.phase1_workflow.scout_stores
  - score_stores   → app.workflows.phase1_workflow.score_stores
"""
...
from app.workflows.phase1_workflow import (
    _scout_dict_to_candidate,
    phase1_graph,
)
...
@pytest.mark.asyncio
async def test_happy_path_all_nodes_run(tmp_path, monkeypatch):
    """Full pipeline: brand → scout → analyst → report. No errors."""
    monkeypatch.chdir(tmp_path)   # report file written to tmp_path/reports/
...
@pytest.mark.asyncio
async def test_report_markdown_content(tmp_path, monkeypatch):
    """Report file contains brand name, tenant, and store count."""
...
@pytest.mark.asyncio
async def test_brand_failure_skips_scout_and_analyst(tmp_path, monkeypatch):
    """When brand extraction fails, scout and analyst are skipped gracefully."""
...
@pytest.mark.asyncio
async def test_scout_failure_skips_analyst(tmp_path, monkeypatch):
    """When scout fails, analyst is skipped but brand profile is preserved."""
    monkeypatch.chdir(tmp_path)
...
@pytest.mark.asyncio
async def test_error_threshold_routes_to_error_handler(tmp_path, monkeypatch):
    """4 pre-seeded errors → error_handler fires after brand_extractor."""
    monkeypatch.chdir(tmp_path)

    inp = _base_input()
    inp["errors"] = ["e1", "e2", "e3", "e4"]   # already 4 > threshold of 3
    ...
    # Error handler fires immediately — scout is never called
    mock_scout.assert_not_called()
    assert result["report_url"] == ""


@pytest.mark.asyncio
async def test_partial_results_on_analyst_failure(tmp_path, monkeypatch):
    """Analyst failure → report still generated with 0 scored stores."""
    monkeypatch.chdir(tmp_path)
    ...
    assert result["report_url"] != ""           # report still written
    assert any("analyst" in e for e in result["errors"])


# ---------------------------------------------------------------------------
# Unit tests for _scout_dict_to_candidate
# ---------------------------------------------------------------------------

def test_scout_dict_to_candidate_full_address():
    """Standard 4-part address parses city and state correctly."""
    d = {
        "google_place_id": "ChIJ001",
        "name": "My Shop",
        "address": "123 Main St, Brooklyn, NY 11201",
...
def test_scout_dict_to_candidate_missing_fields():
    """Missing optional fields default to empty/None without raising."""
...
def test_scout_dict_to_candidate_short_address():
    """Short address (2 parts) parses city and state correctly."""
    d = {"address": "Brooklyn, NY"}
    c = _scout_dict_to_candidate(d)
    assert c.city == "Brooklyn"
    assert c.state == "NY"
